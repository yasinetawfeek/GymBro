...
def load_model():
    ...
    try:
        # Adjust the path based on where the model is actually located
        # Look for the model in multiple potential locations
        model_paths = [
            'AI/data/best_model.pth',
            'data/best_model.pth',
            '../data/best_model.pth',
            '../../data/best_model.pth'
        ]
...
@app.route('/')
def index():
    return "Pose Correction WebSocket Server - Connect with a compatible client"
...
if __name__ == '__main__':
    print("Starting WebSocket server on http://localhost:8001")
