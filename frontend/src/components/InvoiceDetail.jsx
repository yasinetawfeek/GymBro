...
import { 
  ArrowLeft, Calendar, CreditCard, CheckCircle, 
  AlertCircle, Clock, Download, Printer, Share2 
} from 'lucide-react';
import LoadingSpinner from './LoadingSpinner';

const InvoiceDetail = ({ isDarkMode }) => {
  ...
  const handlePayInvoice = async () => {
    ...
    try {
      ...
      const updatedInvoice = await axios.get(`/api/invoices/${invoiceId}/`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('access_token')}`
        }
      });
  ...
  return (
    <div className={`min-h-screen ${isDarkMode ? 'bg-gray-900 text-white' : 'bg-gray-50 text-gray-900'} p-6 print:bg-white print:text-black print:p-0`}>
      ...
      {/* Main invoice card */}
      <div className={`max-w-4xl mx-auto rounded-lg shadow-lg overflow-hidden print:shadow-none print:border ${
        ...
        {/* Payment Section - Only shown for pending invoices and when not printing */}
        {invoice.status === 'pending' && (
          <div className={`p-6 border-t print:hidden ${isDarkMode ? 'border-gray-700' : 'border-gray-200'}`}>
            <h3 className={`text-sm font-medium mb-4 ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
              PAYMENT
            </h3>
            {paymentSuccess && (
        ...
        {/* Actions Footer - hide when printing */}
        <div className={`p-6 border-t print:hidden ${isDarkMode ? 'border-gray-700' : 'border-gray-200'}`}>
